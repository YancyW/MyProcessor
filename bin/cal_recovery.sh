...
out_name=${input_direction}/${input_class_name}_recovery.dat



if [[ $# == 2 ]] ; then
	cal_name=${input_direction}/${input_class_name}_${input_final_state}_recovery.dat
	if [[ -a ${cal_name} ]] ; then
		${DIR}/cal_event ${cal_name}  ${out_name}
		echo "done"
	else
		echo "no such file ${cal_name}"
		exit
	fi
elif [[ $# == 3 ]] ; then
	cal_name=${input_direction}/${input_class_name}_${input_final_state}_recovery.dat
	cal_name2=${input_direction}/${input_class_name}_${input_final_state2}_recovery.dat
