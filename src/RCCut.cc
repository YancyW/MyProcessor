...
bool MyProcessor::RCCutMuon( std::vector<ReconstructedParticle*> &muon,std::vector<ReconstructedParticle*> &muon_plus,std::vector<ReconstructedParticle*> &muon_minus, Variable &data_muon) {
...
bool MyProcessor::RCCutElec( std::vector<ReconstructedParticle*> RCs) {
	int num = RCs.size();
	if(num==0||num==1) {
		return(false);
	}
