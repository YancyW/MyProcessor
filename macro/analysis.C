...
void analysis(std::vector<std::string> arg_string1,std::vector<std::string> arg_string2,std::string out_name) {
	...
	pol.push_back( elecm * posip );
	pol.push_back( elecp * posim );
	pol.push_back( elecm * posim );
	pol.push_back( elecp * posip );
	...
	for(unsigned int i=0;i<filename.size();i++){
		...
		weight.push_back(xection[i]/nevent[i]*pol[i]);
		...
		ShowMessage(3,"passed cut two ISO muon        ", pass_basic_muon [i]);
