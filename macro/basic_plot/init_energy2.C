...
void init_energy2(std::vector<std::string> mhmass) {
	...
	for(int i=0;i<filename.size();i++){
		...
		float variable;
		MyLCTuple->SetBranchAddress("init_energy2", &variable);

		hvariable[i] = new TH1F("init_energy2","init_energy2", 60,70,130);
		hvariable[i]->SetTitle(";init_Energy2(GeV/c^{2});Events");
...
////s->Draw("nostack");
	...
	img->FromPad(c1);
	std::string pngname="init_energy2.png";
