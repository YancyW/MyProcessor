...
void kcut_invis_costheta(std::vector<std::string> mhmass) {
	...
	string title_name="po_muon_kcut_invis_costheta";
	string x_name=";Cos #theta";
	...
	for(int i=0;i<filename.size();i++){
		...
		MyLCTuple->SetBranchStatus(title_name.c_str(), 1);

		hvariable[i] = new TH1F(title_name.c_str(),title_name.c_str(),200,-1,1);
	...
	s->GetXaxis()->SetTitle("Cos#theta_{inv}");
	leg->Draw();
