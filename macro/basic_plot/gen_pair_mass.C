...
void gen_pair_mass(std::vector<std::string> mhmass) {
	...
	for(int i=0;i<filename.size();i++){
		...
		float variable;
		MyLCTuple->SetBranchAddress("gen_pair_mass"  , &variable);MyLCTuple->SetBranchStatus("gen_pair_mass"  ,1);

		hvariable[i] = new TH1F("gen_pair_mass>>h1","pair_mass",100,1,250);
		hvariable[i]->SetTitle(";Pair Mass(GeV/c^{2});Events");
...
//	leg->Draw();
	...
	img->FromPad(c1);
	std::string pngname="gen_pair_mass.png";
