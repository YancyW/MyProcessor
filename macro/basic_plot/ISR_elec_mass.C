...
void ISR_elec_mass(std::vector<std::string> mhmass) {
	...
	auto leg = new TLegend(0.2,0.7,0.35,0.9);
	...
	for(int i=0;i<filename.size();i++){
		...
		float variable;
		MyLCTuple->SetBranchAddress("ISR_elec_mass", &variable);
//		MyLCTuple->SetBranchStatus("mc_ISR_elec_mass",1);

		hvariable[i] = new TH1F("ISR_elec_mass","ISR_elec_mass",100,1,250);
		hvariable[i]->SetTitle(";ISR_elec Mass(GeV/c^{2});Events");
...
////s->Draw("nostack");
	...
	img->FromPad(c1);
	std::string pngname="ISR_elec_mass.png";
