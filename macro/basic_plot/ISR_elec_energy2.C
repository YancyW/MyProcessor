...
void ISR_elec_energy2(std::vector<std::string> mhmass) {
	...
	for(int i=0;i<filename.size();i++){
		...
		float variable;
		MyLCTuple->SetBranchAddress("ISR_elec_energy2", &variable);
//		MyLCTuple->SetBranchStatus("mc_ISR_elec_mass",1);

		hvariable[i] = new TH1F("ISR_elec_energy2","ISR_elec_energy2",100,1,200);
		hvariable[i]->SetTitle(";ISR_elec energy (GeV/c^{2});Events");
...
////s->Draw("nostack");
	...
	img->FromPad(c1);
	std::string pngname="ISR_elec_energy2.png";
