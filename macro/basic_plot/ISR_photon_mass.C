...
void ISR_photon_mass(std::vector<std::string> mhmass) {
	...
	for(int i=0;i<filename.size();i++){
		...
		float variable;
		MyLCTuple->SetBranchAddress("ISR_photon_mass", &variable);
//		MyLCTuple->SetBranchStatus("mc_ISR_photon_mass",1);

		hvariable[i] = new TH1F("ISR_photon_mass","ISR_photon_mass",100,1,200);
		hvariable[i]->SetTitle(";ISR_photon Mass(GeV/c^{2});Events");
...
////s->Draw("nostack");
	...
	img->FromPad(c1);
	std::string pngname="ISR_photon_mass.png";
