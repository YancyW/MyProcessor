...
void ISR_photon_energy2(std::vector<std::string> mhmass) {
	...
	for(int i=0;i<filename.size();i++){
		...
		float variable;
		MyLCTuple->SetBranchAddress("ISR_photon_energy2", &variable);
//		MyLCTuple->SetBranchStatus("mc_ISR_photon_mass",1);

		hvariable[i] = new TH1F("ISR_photon_energy2","ISR_photon_energy2", 50,1,150);
...
////s->Draw("nostack");
	...
	img->FromPad(c1);
	std::string pngname="ISR_photon_energy2.png";
