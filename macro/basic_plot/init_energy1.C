...
void init_energy1(std::vector<std::string> mhmass) {
	...
	for(int i=0;i<filename.size();i++){
		...
		float variable;
		MyLCTuple->SetBranchAddress("init_energy1", &variable);
	//	MyLCTuple->SetBranchStatus("init_energy",1);

		hvariable[i] = new TH1F("init_energy1","init_energy1",60,70,130);
		hvariable[i]->SetTitle(";init_Energy1(GeV/c^{2});Events");
...
////s->Draw("nostack");
	...
	img->FromPad(c1);
	std::string pngname="init_energy1.png";
