...
void kcut_recoil_mass(std::vector<std::string> mhmass) {
	...
	string title_name="po_muon_kcut_recoil_mass";
	...
	for(int i=0;i<filename.size();i++){
		...
		hvariable[i] = new TH1F(title_name.c_str(),title_name.c_str(), 250,0.1,250);
	...
	//for(int i=0;i<filename.size();i++){
	//std::cout<<TMath::GetMaxElement(filename.size())<<std::endl;
	//}
	...
	s->GetXaxis()->SetTitle("M_{rec} GeV/c^{2}");
	leg->Draw();
