...
void pair_mass(std::vector<std::string> mhmass) {
...
//  	gPad->SetLogy();
	...
	for(int i=0;i<filename.size();i++){
		...
		float variable;
		MyLCTuple->SetBranchAddress("mc_pair_mass"  , &variable);
		MyLCTuple->SetBranchStatus("mc_pair_mass"  ,1);

		hvariable[i] = new TH1F("mc_pair_mass>>h1","pair_mass",100,1, 250);
		hvariable[i]->SetTitle(";#mu^{+}#mu^{-} Pair Mass(GeV/c^{2});Events");
...
////s->Draw("nostack");
	...
	img->FromPad(c1);
	std::string pngname="pair_mass.png";
