...
void analysis_total(std::vector<std::string> arg_string1,std::string arg_string3, std::vector<std::string> arg_string2,std::string out_name,float mass) {
	...
	//plot
    int k=0;
    string mass_str=Num_To_String(mass);
    for(unsigned int j=0;j<title_name.size();j++){
    	if(plot_switch[j]){
    		...
    		img->FromPad(c1[k]);
    		std::string pngname="../plot_"+mass_str+"/"+title_name[j]+mass_str+".png";
